...
from criterion import my_log_loss
...
class DKN_R():
    ...
    def TXl(self, X, b_left, b_right, l):
        d_1, p_1 = np.prod(self.bs_ext[:l+1, :], axis=0)
        dl, pl = self.bs_ext[l, :]
        d_L, p_L = np.prod(self.bs_ext[l+1:, :], axis=0)

        R1 = R_opt_pro(X, [d_L, p_L])
        M1 = Vec_inv(b_left.T.dot(R1), [d_1, p_1])
        X_tilde = R_opt_pro(M1, [dl, pl]).dot(b_right)
    ...
    def Init(self, X, Y, R, right=False):
        ...
        for l in range(2, self.L + 1):
            ...
            M = np.sum([Y[i] * R_opt_pro(X[i], shape_w_L) for i in range(len(Y))], axis=0)
    ...
    def Init_random(self, R, right=False, seed=0):
        ...
        for l in range(2, self.L + 1):
            shape_w_L = np.prod(self.bs_ext[l:, :], axis=0)
            b_l = np.random.normal(size=(np.prod(shape_w_L), R))
            b2l_list[l - 2] = b_l
        b2l_list[-1] = np.ones([1, R])
    ...
    def fit(self, train, valid, init='svd', alpha=0, max_itr=10, tol=1e-5, timing=False):
        ...
        if self.task == 'reg':
            ...
            raise ValueError('Task should be regression (reg) or classification (cla).')
        X, Y = train
        X_val, Y_val = valid
        Y = Y.reshape(-1)
        b_list = [np.array([])] * self.L
        b_list.insert(0, np.array([[1]]))
        b_list.append(np.array([[1]]))  # length of b_list: L+2, from 0 to L+1.
        if self.from_init:
        ...
        for itr in range(max_itr):
            ...
            # Validation Error
            Y_val_hat = np.array([np.sum(np.vdot(Xi, C_hat)) for Xi in X_val]).reshape(-1, 1)
            if self.task == 'reg':
            ...
            if change_rate < tol:
                ...
                break
            if loss_val < 10 * tol:
                print('Validation loss vanishes at iteration {}.'.format(itr))
                break
